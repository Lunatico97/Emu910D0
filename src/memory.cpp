...
void Memory::store(u16 m_address, u8 value)
{
    ...
    else if(m_address >= 0x2000 && m_address < 0x4000) ppu->write_from_cpu(m_address, value);
    else if(m_address >= 0x4000 && m_address < 0x4020);
...
u8 Memory::retreive(u16 m_address)
{
    ...
    else if(m_address >= 0x2000 && m_address < 0x4000) return ppu->read_from_cpu(m_address);
