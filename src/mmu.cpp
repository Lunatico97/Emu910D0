...
void MMU::perform_audio_dma(bool sync_rw)
{
    if(!dmc_sync)
    {
        if(sync_rw) dmc_sync = true;
        return;
    }
    
    if(!sync_rw)
    { 
        apu->refDMC->buffer = retreive(apu->refDMC->cur_addr);
        ...
        if(--apu->refDMC->byte_rem == 0x00)
        {
            ...
            {
                apu->refDMC->cur_addr = apu->refDMC->smp_addr;
                apu->refDMC->byte_rem = apu->refDMC->smp_len;
            }
            else if(apu->refDMC->dmc_int)
            {
                dma_irq = true;
            }
        }
